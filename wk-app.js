(function () {
  ...
  const MODE_META = {
    walk: { label: "Gehen", icon: "walk", maxPlausibleKmh: 20, color: "#3E8B74" },
  ...
  /* ---------- Leaflet: Einwilligungspflichtiges Nachladen vom CDN ----------
     Leaflet wird NICHT mehr statisch im <head> geladen, sondern erst dann von
     unpkg.com (CDN) nachgefragt, wenn der Nutzer die Kartenansicht aktiv
     nutzen möchte UND dem einmalig zugestimmt hat (Speicherung der Wahl in
     localStorage, danach kein erneutes Fragen). Ohne Zustimmung bleibt die
     App voll funktionsfähig – Aufzeichnung, Zeit, Strecke, Tempo hängen nie
     von der Karte ab, es erscheint nur ein Hinweistext statt der Karte. */
  const LEAFLET_CSS_URL = "https://unpkg.com/leaflet@1.9.4/dist/leaflet.css";
  const LEAFLET_CSS_INTEGRITY = "sha256-p4NxAoJBhIIN+hmNHrzRCf9tD/miZyoHS5obTRR9BMY=";
  const LEAFLET_JS_URL = "https://unpkg.com/leaflet@1.9.4/dist/leaflet.js";
  const LEAFLET_JS_INTEGRITY = "sha256-20nQCchB9co0qIjJZRGuk2/Z9VM+kNiyxNV1lvTlZBo=";
  const MAP_CONSENT_KEY = "wk_map_consent"; // "granted" | "declined"
  ...
  function updateNavInfo() {
    ...
    if (navRouteCoords && navRouteCoords.length > 1) {
      ...
      if (curStep) {
        ...
        // Restzeit: Die gespeicherten Schritt-Dauern decken nur die Strecke AB
        // dem jeweiligen Manöverpunkt ab, nicht das Stück von der aktuellen
        // Position bis dahin (distToManeuver). Bei einem langen, abbiegefreien
        // Schlussabschnitt bleibt oft nur noch der reine Ziel-Schritt übrig
        // (distance/duration = 0) – ohne Umrechnung von distToManeuver würde
        // die Restzeit dann fälschlich auf 0 springen, obwohl noch echte
        // Distanz übrig ist. Deshalb wird distToManeuver mit dem
        // Durchschnittstempo der Gesamtroute (navRoutePaceSecPerM) in eine
        // Zeit umgerechnet und zu den restlichen Schritt-Dauern addiert.
        const maneuverEtaSec = navRoutePaceSecPerM != null ? distToManeuver * navRoutePaceSecPerM : 0;
        etaSec = maneuverEtaSec + steps.slice(navStepIndex).reduce((s, st) => s + (st.duration || 0), 0);
      }
  ...
  function updateNewTourHint() {
    if (sessionType === "track") {
      $("newtourHint").textContent = "Modus wählen und auf „Start” tippen. Aufzeichnung startet erst mit diesem Tipp und läuft nur, solange die App geöffnet ist.";
