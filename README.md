...
- **Restzeit-Anzeige während der Navigation korrigiert**: Bei einem
  langen, abbiegefreien Schlussabschnitt (keine weiteren Manöver bis
  zum Ziel) zeigte die Restzeit fälschlich „0h 0m" an, obwohl noch
  reale Distanz übrig war – die Restdistanz war dabei korrekt, weil
  sie per GPS-Entfernung zum Zielpunkt berechnet wird, die Restzeit
  aber ausschließlich aus den (in diesem Fall leeren) OSRM-
  Schrittdauern stammte. Die Zeit bis zum nächsten Manöverpunkt wird
  jetzt zusätzlich über das Durchschnittstempo der Gesamtroute
  geschätzt und eingerechnet.
- **Tour-/Zusammenfassungskarte robuster gegen „bleibt leer"**:
  Kartenaufbau in Zusammenfassung und Tour-Detail prüft jetzt über
  mehrere gestaffelte Nachversuche und einen ResizeObserver, ob der
  Kartencontainer seine endgültige Größe erreicht hat, statt sich auf
  einen einzelnen verzögerten Neuberechnungs-Aufruf zu verlassen.
  Ziel: die wiederholt gemeldete unsichtbare Karte beim Ansehen einer
  gespeicherten Tour im Verlauf zuverlässig beheben.
- Navigations-Ablauf (Abbiege-Hinweise, Neuberechnung bei Abweichen
  von der Route, Zeitpunkt der Signale) sowie die Rechts-/Lizenztexte
  wurden komplett durchgesehen – hier wurde kein weiterer Fehler
  gefunden, die bestehende Logik (Schwellenwerte aus v40/v41/v44/v45)
  ist in sich stimmig.
